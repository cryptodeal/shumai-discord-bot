...
const sync = async (
	...
	const res = await fetch(
		`${RouteBases.api}${
			guildId
				? Routes.applicationGuildCommands(clientUserId, guildId)
	...
	if (res.status === 429) {
		setTimeout(() => sync(clientToken, clientUserId, commands, guildId), data.retry_after * 1000);
	} else {
		Logger.error(typeof data.code !== 'undefined' ? data.code.toString() : '', data.message);
