...
try {
	...
	requestGithub(`issues/${pullRequestNumber}/labels`, {
		labels: ['tags']
	});
} catch (e) {
	tags = [];
...
for (const [key, value] of Object.entries(tags)) {
	...
	if (tag?.keywords?.[0] !== key)
		errors.push(`**[${key}]:** First keyword of tag is not the same as the tag name`);
...
if (errors.length === 0) {
	requestGithub(`pulls/${pullRequestNumber}/reviews`, {
		commit_id: commitSha,
		event: 'APPROVE'
	});

	requestGithub(`pulls/${pullRequestNumber}/requested_reviewers`, {
		reviewers: ['xHyroM']
	});

	requestGithub(`issues/${pullRequestNumber}/labels/waiting`, {}, 'DELETE');
