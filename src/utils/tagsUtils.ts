...
export const getTag = <T extends boolean>(
	name: string,
	more?: T
): T extends true ? APIApplicationCommandOptionChoice[] : Tag => {
	if (more) {
		...
		const tag =
			tagCache.get(name) || tagCache.find((tag) => tag.keywords.some((k) => k.includes(name)));
